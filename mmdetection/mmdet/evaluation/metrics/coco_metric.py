...
class CocoMetric(BaseMetric):
    ...
    def results2json(self, results: Sequence[dict],
        ...
        num_cats = len(self.cat_ids) if self.cat_ids is not None else 0
        _warned_oob = False
        for idx, result in enumerate(results):
            ...
            # bbox results: clamp out-of-range labels so evaluation can run (e.g. UODD 3-class)
            for i, label in enumerate(labels):
                if self.cat_ids is None:
                    continue
                if num_cats == 0:
                    continue
                if label < 0 or label >= num_cats:
                    if not _warned_oob:
                        MMLogger.get_current_instance().warning(
                            f'CocoMetric: prediction label {int(label)} out of range [0, {num_cats - 1}], '
                            'clamping to valid range for this bbox.')
                        _warned_oob = True
                    label = min(max(0, int(label)), num_cats - 1)
                data = dict()
                data['image_id'] = int(image_id)
                data['bbox'] = self.xyxy2xywh(bboxes[i])
                data['score'] = float(scores[i])
                data['category_id'] = int(self.cat_ids[label])
                bbox_json_results.append(data)

            if segm_json_results is None:
                continue

            # segm results (same label clamping as bbox)
            masks = result['masks']
            mask_scores = result.get('mask_scores', scores)
            for i, label in enumerate(labels):
                if self.cat_ids is None or num_cats == 0:
                    continue
                if label < 0 or label >= num_cats:
                    label = min(max(0, int(label)), num_cats - 1)
                data = dict()
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if self.cat_ids is None:
            ...
            if len(self.cat_ids) == 0:
                # fallback: ann file categories may use different names/order
                self.cat_ids = self._coco_api.get_cat_ids()
                logger.warning(
                    'CocoMetric: get_cat_ids(cat_names=...) returned empty, '
                    'using all category ids from ann file.')
        ...
        for metric in self.metrics:
            ...
            try:
                ...
                valid_img_ids = set(self._coco_api.getImgIds())
                predictions = [x for x in predictions if x['image_id'] in valid_img_ids]
                if iou_type == 'segm':
