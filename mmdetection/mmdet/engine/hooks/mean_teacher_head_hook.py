...
class MeanTeacherHeadHook(Hook):
    """Mean Teacher Hook.
    ...
    def before_train(self, runner: Runner) -> None:
        ...
        assert hasattr(model, 'bbox_head_teacher')
    ...
    def momentum_update(self, model: nn.Module, momentum: float) -> None:
        ...
        if self.skip_buffers:
            for (src_name, src_parm), (dst_name, dst_parm) in zip(
                    model.bbox_head.named_parameters(),
                    model.bbox_head_teacher.named_parameters()):
                dst_parm.data.mul_(1 - momentum).add_(
                    src_parm.data, alpha=momentum)
        else:
            for (src_parm,
                 dst_parm) in zip(model.bbox_head.state_dict().values(),
                                  model.bbox_head_teacher.state_dict().values()):
